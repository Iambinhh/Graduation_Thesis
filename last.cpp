    ...
    while (ros::ok())
    {
        ...
        if(start)
        {
            ...
            else if (time_count>=4) 
            {
                ...
                {
                    max_angle_ = 0.2;
                    desired_local_position_.y = desired_local_position_.y+sin(desired_local_position_yaw)*(desired_local_position_.z+2);
                    //desired_local_position_.z*tan(45) = khoảng cách từ máy bay đến lửa
                    desired_local_position_.x = desired_local_position_.x+cos(desired_local_position_yaw)*(desired_local_position_.z+2);
                    tien = 2;
                    ready = ros::Time::now();
                    msg_motor.data = 0;
                }
